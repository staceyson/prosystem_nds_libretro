...
/* Rocketfan notes - This was originally written for NDS systems which had only 4 or 16MB of RAM available.
                     Specifically it used some chunks af available NDS VRAM in actual NDS
                     hardware banks for that purpose.
                     For a modern emulation system, I believe we should be able to just allocate a big memory
                     array and maybe just adjust pointer(s) into that to simulate bank switching?

                     "Presently 2x128K Banksets are the largest supported carts."  According to:
                      https://7800.8bitdev.org/index.php/Bankset_Bankswitching

                     memory_ram is a 64K big "normal" buffer.  TBH (for the sake of emulation) I'm not sure
...
ITCM_CODE void cartridge_WriteBank(word address, byte bank) 
{
  if (myCartInfo.cardtype == CARTRIDGE_TYPE_SUPERCART_RAMX2) // Banking RAM
  {
    ...
    {
// Rocket - the shadow ram concept copies the cart ROM to fast RAM on the NDS, which maybe is useless
//          for emulation on other devices??

        u32 *dest = (u32*)(memory_ram+0x4000);
        u32 *src  = ex_ram_bank ? (u32*)(ex_ram_buffer+0x0000) : (u32*)(ex_ram_buffer+0x4000);  // 
        shadow_ram = ex_ram_bank ? (u8*)(ex_ram_buffer+0x0000) : (u8*)(ex_ram_buffer+0x4000);  // 

        shadow_ram -= 0x4000; // Makes for faster indexing in Memory.c  Rocket - huh?  I will just go with it..
...
// Rocket - this is going to have to be all standard memory for a non-DS implementation
//  previous emulator was using unused VRAM bank memory from NDS, @bank B and Bank D...
...
// Rocket - see notes above - we act like a ds lite in that we don't use the VRAM.

        u32 *dest = (u32*)(memory_ram+0x4000);
        u32 *src  = ex_ram_bank_df ? (u32*)(ex_ram_buffer+0x0000) : (u32*)(ex_ram_buffer+0x4000);  // 
...
bool cartridge_Load(const char *filename) 
{
    ...
    if(file == NULL)  {
        ...
        fprintf(stdout, "error on fopen\n");  // Yeah, this should be log_cb for a libretro core.
        fflush(stdout);
        return false;
    }  
    ...
    if (size > (MAX_CART_SIZE+128)) {
        fclose(file); 
        fprintf(stdout, "file too large %d\n", size);
        fflush(stdout);
        return false;   // Cart is too big... can't handle it  
    }
...
void bios_check_and_load(char *defaultLibretroBIOSPath)
{
    bios_available = 0;
